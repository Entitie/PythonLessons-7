...
class loto_card:
    ...
    def __init__(self):
        ...
        card = random.sample([1, 1, 1, 1, 1, '', '', '', ''], 9) + random.sample([1, 1, 1, 1, 1, '', '', '', ''],
                                                                                 9) + random.sample(
            [1, 1, 1, 1, 1, '', '', '', ''], 9)
        i = 0
...
class loto_game:
    ...
    def move(self):
        ...
        print('Новый' if len(self.moves_done) else 'Первый',
              'бочонок: {} (осталось {})'.format(x, len(self.moves_left)))
        ...
        if not self.finished and (not self.computer_card.numbers_left or not self.player_card.numbers_left):
            ...
            self.finished = True
        message = print(
            "Играем дальше!\nОсталось чисел:\nУ Вас - {} \nУ компьютера - {}\n\n".format(self.player_card.numbers_left,
                                                                                         self.computer_card.numbers_left)) if not self.finished else False
    ...
    def print_cards(self):
        ...
        for j in range(3):
            player_line = ' '.join(
                ['{:>2}'.format(self.player_card.order[j * 9 + i][0]) if self.player_card.order[j * 9 + i][1] else ' -'
                 for i in range(9)])
            computer_line = ' '.join(['{:>2}'.format(self.computer_card.order[j * 9 + i][0]) if
                                      self.computer_card.order[j * 9 + i][1] else ' -' for i in range(9)])
